...
def ValueIterations(V, P_hat, rewards):
        ...
        for arm_id in range(V.shape[0]):
            for s in range(V.shape[1]):
                V_new[arm_id, s] = np.max(
                    (rewards[arm_id][s] + 0.99*V[arm_id, :]) @ P_hat[arm_id, s, :, :]
                )
...
def ValueIteration(V_arm, P_hat_sub, rewards_sub):
    ...
    while delta > convergence_thresh:
        ...
        for s in range(V_arm.shape[0]):
            V_new[s] = np.max((rewards_sub[s] + 0.99*V_arm) @ P_hat_sub[s, :, :])
...
@register_training_function("sub_sample")
def train(env, cfg):
    ...
    Z_s = np.zeros((num_arms, num_states), dtype=int)  # (N_k_t in paper; num_sub_samples)
    ...
    transitions = {arm_id: deque(maxlen=cfg["history_size"]) for arm_id in range(num_arms)}
            ...
            for action in range(num_actions):
                ...
                for arm_id, s_dash in zip(range(num_arms), s_dash_list):
                    reward_averages[arm_id, state] = update_moving_average(
                        reward_averages[arm_id, state], rewards_list[arm_id], Z_s[arm_id, state] + 1
                    )
                    Z_s[arm_id, state] += 1
    ...
    for k in tqdm(range(K)):
        ...
        for t in range(env.H):
            ...
            V = ValueIterations(V, P_hat, global_reward_est)
                ...
                for leader_arm in range(num_arms):
                    ...
                    if n_leader >= n_contender:  # Subsample leader arm
                        ...
                        V_sub = ValueIteration(
                            V[leader_arm, :], P_hat_sub, rewards_sub
                        )
                        ...
                        V_sub = ValueIteration(
                            V[contender_arm, :], P_hat_sub, rewards_sub
                        )
                        W_sub = ComputeLambda(V_sub, P_hat_sub, rewards_sub)
                        if (
